...
from .yaml import import_from_yaml
                ...
                for transition in transitions:
                    ...
                    self.output(
                        "{} : {}/ {}".format(
                            self.state_id(name),
                            "".join(text),
                            cast(str, transition.action).strip().replace("\n", "; "),
                        ),
                    )
...
def export_to_plantuml(
    ...
    *,
    based_on: str | None = None,
    based_on_filepath: str | bytes | os.PathLike | None = None,
    statechart_name: bool = True,
...
def cli(args=None) -> int:
    ...
    args, _parameters = parser.parse_known_args(args)
